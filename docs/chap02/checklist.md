...
## 实现完整性
- [ ] 配置加载：合法 `.nuocode/config.yaml` 能解析出 providers 列表（验证：单测 + 启动进入对话）。(AC1/F1)
- [ ] 配置校验：缺密钥/非法 protocol/文件缺失时给出可读错误并非零退出，无未捕获堆栈（验证：删字段/改 protocol/删文件分别运行 `python -m nuocode`）。(AC1/N4)
