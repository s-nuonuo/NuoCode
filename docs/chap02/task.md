...
> 包名：`nuocode`（Python 3.12+）。源码位于 `src/nuocode/`，内部模块以 `nuocode.xxx` 导入。
...
| 新建 | `.nuocode/config.yaml.example` | 配置模板 |
| 修改 | `.gitignore` | 忽略 `.nuocode/config.yaml` |
| 新建 | `src/nuocode/__init__.py` | 包标识、版本号 `__version__` |
| 新建 | `src/nuocode/__main__.py` | `python -m nuocode` 入口（转调 `cli.main`） |
| 新建 | `src/nuocode/config.py` | `Config` / `ProviderConfig`、`load`、校验 |
| 新建 | `src/nuocode/prompt.py` | `SYSTEM_PROMPT`、`CAT_BANNER`、`render_banner` |
| 新建 | `src/nuocode/llm/__init__.py` | `Provider` Protocol、`Message`、`StreamEvent`、`new_provider` 工厂 |
| 新建 | `src/nuocode/conversation.py` | 单会话多轮历史 |
| 新建 | `src/nuocode/llm/anthropic_provider.py` | anthropic 适配器 |
| 新建 | `src/nuocode/llm/openai_provider.py` | openai 适配器 |
| 新建 | `src/nuocode/tui/__init__.py` | TUI 包标识 |
| 新建 | `src/nuocode/tui/app.py` | `nuocodeApp`、状态机、`run` |
| 新建 | `src/nuocode/tui/stream.py` | `_consume_stream`、`_tick` 计时 |
| 新建 | `src/nuocode/tui/select.py` | provider 选择（`OptionList`） |
| 新建 | `src/nuocode/tui/view.py` | 渲染拼装、状态栏、错误样式、markdown 定型 |
| 新建 | `src/nuocode/cli.py` | 入口装配 |
...
**文件：** `pyproject.toml`、`src/nuocode/__init__.py`、`src/nuocode/__main__.py`、`src/nuocode/cli.py`（临时占位）
**依赖：** 无
**步骤：**
1. 用 `uv init` 或手写 `pyproject.toml`，关键字段：
   ...
   packages = ["src/nuocode"]

   [dependency-groups]
   dev = ["pytest>=8", "ruff>=0.6", "mypy>=1.10"]
   ```
2. `src/nuocode/__init__.py`：定义 `__version__ = "0.1.0"`。
3. `src/nuocode/__main__.py`：`from .cli import main; main()`。
4. `src/nuocode/cli.py` 写一个临时 `main()`，打印 `f"nuocode {__version__}"` 并退出，确保可启动。
5. 安装依赖：`uv sync`（推荐）或 `pip install -e ".[dev]"`。

**验证：** `python -m nuocode` 能打印版本号；`uv run nuocode`（或 `nuocode`）同样可用；`uv pip list` / `pip list` 能看到上述依赖。

## T2: config 模块
**文件：** `src/nuocode/config.py`、`tests/test_config.py`
**依赖：** T1
**步骤：**
...
**验证：** `pytest tests/test_config.py` 通过；`ruff check src/nuocode/config.py` 无告警。

## T3: 配置模板与忽略
**文件：** `.nuocode/config.yaml.example`、`.gitignore`
**依赖：** T2
**步骤：**
1. 写 `.nuocode/config.yaml.example`：含 anthropic 条目（含 `thinking: true`）与一段注释掉的 openai 条目示例，字段与 `ProviderConfig` 对齐。
2. `.gitignore` 追加 `.nuocode/config.yaml`。

**验证：** 复制 example 为 `.nuocode/config.yaml` 后 `config.load(...)` 通过；`git status` 确认 `.nuocode/config.yaml` 被忽略。

## T4: prompt 模块
**文件：** `src/nuocode/prompt.py`
**依赖：** T1
**步骤：**
1. 定义 `SYSTEM_PROMPT: str = """..."""`（一段简洁的固定 system prompt）。
2. 定义 `CAT_BANNER: str = """..."""`（ASCII 猫：`/\\_/\\`、`( o.o )`、`> ^ <`）。
3. 实现 `def render_banner(version: str, cwd: str) -> str`：拼出"猫 + nuocode vX + cwd + 就绪提示行"。

**验证：** `python -c "from nuocode.prompt import render_banner; print(render_banner('0.1.0', '/tmp'))"` 输出含三要素与提示行。

## T5: llm 包骨架
**文件：** `src/nuocode/llm/__init__.py`
**依赖：** T2
**步骤：**
...
**验证：** `python -c "from nuocode.llm import Provider, Message, StreamEvent, new_provider"` 不报错。

## T6: conversation 模块
**文件：** `src/nuocode/conversation.py`、`tests/test_conversation.py`
**依赖：** T5
**步骤：**
...
## T7: anthropic 适配器
**文件：** `src/nuocode/llm/anthropic_provider.py`
**依赖：** T5、T4
**步骤：**
...
**验证：** `python -c "from nuocode.llm.anthropic_provider import AnthropicProvider"` 不报错；联调留到 T14；可写小脚本用假 key 触发错误，确认拿到 `err` 事件。

## T8: openai 适配器
**文件：** `src/nuocode/llm/openai_provider.py`
**依赖：** T5、T4
**步骤：**
...
## T9: TUI App 骨架
**文件：** `src/nuocode/tui/app.py`、`src/nuocode/tui/__init__.py`
**依赖：** T1、T2、T5、T6
**步骤：**
1. 定义 `class SessionState(Enum)`：`SELECTING` / `IDLE` / `STREAMING`。
2. 定义 `class nuocodeApp(App)`：构造参数 `providers: list[ProviderConfig]`；初始化 `state`、`provider: Provider | None`、`conv = Conversation()`、`cur_reply = ""`、`turn_start = 0.0`、`_stream_task = None`、`_timer = None`。
...
6. `def main()`（在 `cli.py` 中调用）：`nuocodeApp(providers).run()`。

**验证：** `python -m nuocode`（搭配最小合法配置）能进入界面，看到 banner + 空对话区 + 输入框 + 状态栏；`ruff check src/nuocode/tui/app.py` 无告警。

## T10: TUI 流式接入与计时
**文件：** `src/nuocode/tui/stream.py`、`src/nuocode/tui/app.py`
**依赖：** T9、T5
**步骤：**
1. 在 `app.py` 给 `nuocodeApp` 添加 `async def submit(self, text: str)`：
...
3. 状态栏：用 Rich 的 `Text`/`Table.grid` 左 `provider.name`、右 `provider.model`，两端对齐；
   写到 `#statusbar: Static`。
4. 完成块（追加到 `RichLog`）：
   - `user_block(text)` = `Text("● " + text, style="bold")` 或纯文本（无 You/nuocode 文字标签）；
...
**验证：** 把工具栏、状态栏、错误样式截图比对；`ruff check src/nuocode/tui/` 无告警。

## T13: 入口装配
**文件：** `src/nuocode/cli.py`（替换 T1 占位）
**依赖：** T2、T4、T9
**步骤：**
1. `def main() -> None`：
   - `try: cfg = config.load(".nuocode/config.yaml")`；`except ConfigError as e: print(e, file=sys.stderr); sys.exit(1)`。
   - 可选：先 `print(prompt.render_banner(__version__, os.getcwd()))`，或交给 TUI 在 `on_mount` 写 `RichLog`（二选一保持一致；本项目用后者）。
   - `nuocodeApp(cfg.providers).run()`；若抛非 KeyboardInterrupt 异常，`print(...)` 并 `sys.exit(1)`。

**验证：** `python -m nuocode` 在合法配置下能启动 TUI；缺配置时打印可读错误并退出码非零。
